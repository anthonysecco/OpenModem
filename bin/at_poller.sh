...
# is pruned of samples older than MIMO_MAX_WINDOW_S (300s / 5min default,
# config/openmodem.conf) and the reported max is taken over whatever
# survives — so a peak fades out of the display this long after it was
# observed even on a carrier that never drops, not just after an absence
# timeout. A key with no samples left after pruning is dropped entirely.
# This also gives a carrier that vanishes from the live reading for a
# while (idle SCC, brief handover away and back to the same cell) its
# last-observed max for free, for as long as that sample is still inside
# the window — no separate "keep it cached across a gap" logic needed,
# since the history simply isn't pruned away yet.
#
